...
namespace Mayfly
{
	public class MayflyModule : InteractionModuleBase<SocketInteractionContext>
	{
		...
		{
			await Context.Interaction.RespondAsync(Format.Sanitize(text), embeds, isTTS, ephemeral, allowedMentions, components, embed, options);
		}
		...
		{
			await Context.Interaction.RespondAsync(Format.Code(code, lang), embeds, isTTS, ephemeral, allowedMentions, components, embed, options);
		}
		...
		{
			await Context.Interaction.FollowupAsync(Format.Sanitize(text), embeds, isTTS, ephemeral, allowedMentions, components, embed, options);
		}
		...
		{
			await Context.Interaction.FollowupAsync(Format.Code(code, lang), embeds, isTTS, ephemeral, allowedMentions, components, embed, options);
		}
		...
		{
			await Context.Interaction.FollowupWithFileAsync(path, filename, text, embeds, isTTS, ephemeral, allowedMentions ?? AllowedMentions.None, components, embed, options ?? RequestOptions.Default);
		}
		...
		{
			await Context.Interaction.FollowupWithFileAsync(stream, filename, text, embeds, isTTS, ephemeral, allowedMentions ?? AllowedMentions.None, components, embed, options ?? RequestOptions.Default);
		}
	}
}